...
mod tests {
    ...
    #[test]
    fn registers_and_enumerates_font_families() {
        let catalog = FontCatalog::new(SystemFonts::Skip);
        catalog
            .register_fonts(vec![Cow::Borrowed(IBM_PLEX), Cow::Borrowed(LILEX)])
            .unwrap();
    ...
    #[test]
    fn resolves_a_registered_face_and_checks_character_coverage() {
        let catalog = FontCatalog::new(SystemFonts::Skip);
        catalog
            .register_fonts(vec![Cow::Borrowed(IBM_PLEX)])
            .unwrap();
    ...
    #[test]
    fn resolves_weight_and_style_together() {
        let catalog = FontCatalog::new(SystemFonts::Skip);
        catalog
            .register_fonts(vec![
        ...
        let face = catalog
            ...
            .unwrap();
        assert_eq!(face.data.as_ref(), IBM_PLEX_SEMIBOLD_ITALIC);
    }

    #[test]
    fn rejected_registration_does_not_advance_generation() {
        let catalog = FontCatalog::new(SystemFonts::Skip);
        assert!(
            catalog
