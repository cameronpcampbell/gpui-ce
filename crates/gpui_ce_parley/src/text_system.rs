use crate::{
    CatalogState, FaceFamily, FaceRequest, FontCatalog, FontRasterizer, FontStore, SystemFonts,
};
use anyhow::{Context as _, Result};
use gpui::{
    Bounds, CaretAffinity, CaretPosition, Font, FontId, FontMetrics, GlyphId, LineLayout,
    PaintFragment, PaintStyle, Pixels, PlatformTextLayout, PlatformTextSystem, RasterizedGlyph,
    RenderGlyphParams, ShapedGlyph, Size, TextLayoutRequest, TextMovement, TextRun,
...
/// Shapes with Parley and paints font instances with Skrifa and Swash.
pub struct ParleyTextSystem {
    catalog: FontCatalog,
    fonts: RwLock<FontStore>,
    rasterizer: Mutex<FontRasterizer>,
...
impl ParleyTextSystem {
    ...
    pub fn new_with_system_font(
        ...
        Self {
            ...
            rasterizer: Mutex::new(FontRasterizer::default()),
...
impl PlatformTextSystem for ParleyTextSystem {
    ...
    fn rasterize_glyph(&self, params: &RenderGlyphParams) -> Result<RasterizedGlyph> {
        ...
        let font = fonts
            .get(params.font_id)
            .context("Parley FontId missing from its store")?;
        self.rasterizer.lock().rasterize_glyph(font, params)
    }
...
mod tests {
    ...
    fn styled_document_uses_parley_paint_runs_without_changing_geometry() {
        ...
        for fragment in &decorated.paint_fragments {
            for glyph in &fragment.glyphs {
                let raster = system
                    .rasterize_glyph(&RenderGlyphParams {
                        font_id: fragment.font_id,
                        glyph_id: glyph.id,
                        font_size: decorated.font_size,
    ...
    #[test]
    fn font_registration_invalidates_layouts_and_preserves_font_instances() {
        let backend = Arc::new(
            ParleyTextSystem::new_with_system_font(SystemFonts::Skip, "IBM Plex Sans")
        ...
        let generation = backend.font_generation();
        backend
            .add_fonts(vec![Cow::Borrowed(SOURCE_SERIF)])
            .unwrap();
        assert!(backend.font_generation() > generation);
        ...
        assert!(
            backend
                .add_fonts(vec![Cow::Borrowed(b"not a font")])
                .is_err()
        );
        assert_eq!(backend.font_generation(), generation);
        ...
        let fonts = backend.fonts.read();
        let variable = fonts.get(variable_id).unwrap();
        assert!(!variable.normalized_coords.is_empty());
        assert!(variable.synthesis.skew().is_some());
    }
    ...
    #[test]
    fn monochrome_and_color_glyphs_rasterize_once_with_complete_geometry() {
        let system = test_system();
        for (text, expect_color) in [("A", false), ("😀", true)] {
            ...
            let raster = system
                .rasterize_glyph(&RenderGlyphParams {
                    ...
                    subpixel_variant: Default::default(),
                    scale_factor: 1.0,
                    is_emoji: glyph.1.is_emoji,
                    subpixel_rendering: false,
                })
                .unwrap();
            assert_eq!(raster.bounds.size, raster.size);
            assert!(raster.size.width.0 > 0 && raster.size.height.0 > 0);
            let channels = if expect_color { 4 } else { 1 };
            assert_eq!(
                raster.pixels.len(),
